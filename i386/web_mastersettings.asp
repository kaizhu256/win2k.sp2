...
%>
		...
		CELLPADDING=2 class="TasksBody">
    			...
    			<TR>
					...
					<TD>
						&nbsp;<INPUT TYPE=TEXT NAME=txtWebRootDir class="formField" tabIndex=1 VALUE="<%=Server.HTMLEncode(F_strWebRootDir)%>" SIZE=30 TABINDEX=1 onKeyPress = "ClearErr()">
					</TD>
    			</TR>
    			...
    			<TR>
    				<TD>
    					<% if F_strIndexResource = true then%> 									
    						<INPUT TYPE=checkbox NAME=chkIndexResource class="formField" checked tabIndex=7 VALUE="<%=Server.HTMLEncode(F_strIndexResource)%>" >
    					<% else %>
    						<INPUT TYPE=checkbox NAME=chkIndexResource class="formField" tabIndex=7 VALUE="<%=Server.HTMLEncode(F_strIndexResource)%>" >
    					<% end if %>
    			...
    			<TR>
    				...
    				<% if F_strMaxCon = L_LIMITEDTOTEXT then %>
    					...
    					&nbsp;&nbsp;<INPUT TYPE=TEXT NAME=txtLimitedTo class="formField" tabIndex=10 VALUE="<%=Server.HTMLEncode(F_strLimitedTo)%>" SIZE=30 OnKeyUP="javaScript:checkUserLimit(this,'con');" onblur = "checkforEmptyfield();" OnKeypress="javaScript:checkKeyforNumbers(this);">
    				<%else%>
    					...
    					&nbsp;&nbsp;<INPUT TYPE=TEXT NAME=txtLimitedTo disabled class="formField" onblur = "checkforEmptyfield();" tabIndex=10 VALUE="<%=Server.HTMLEncode(F_strLimitedTo)%>" SIZE=30 OnKeyUP="javaScript:checkUserLimit(this,'con');" OnKeypress="javaScript:checkKeyforNumbers(this);">
    				<%end if%>
    				</TD>
    			</TR>
		</TABLE>
