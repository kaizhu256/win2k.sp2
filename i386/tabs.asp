...
Public Function SA_ServeTabBar(ByVal strTabContainer)
    ...
    LogOffTabText = ""
    Set objLocMgr = Server.CreateObject("ServerAppliance.LocalizationManager")
    LogOffTabText = objLocMgr.GetSPString("&H400011F8")
    Set ObjLocMgr = nothing

    if LogOffTabText <> "" then
        sNewTab = "<td nowrap class=""" & "InActiveTab" & """>&nbsp;"
        sNewTab = sNewTab & "<a id=LogOutLink onclick=""" + "window.top.close(); "" "
        sNewTab = sNewTab & "class=""" & "InActiveTab" & "NoBorder"" " 
        sNewTab = sNewTab & " title=""" + LogOffTabText + """ "
        sNewTab = sNewTab & " onmouseout=""window.status=''; this.className='" & "InActiveTab" & "NoBorder';return true;"" "
        sNewTab = sNewTab & " onmouseover=""window.status='" + LogOffTabText + "';  this.className='" + "InActiveTab" + "NoBorderHover" + "'; return true;"" "
        sNewTab = sNewTab & ">" & LogOffTabText
        sNewTab = sNewTab & "</a>&nbsp;</td>"
        sNewTab = sNewTab & "<td width=2 class=""" & "InActiveTab" & """>"
        sNewTab = sNewTab & "<IMG SRC='" + m_VirtualRoot + "images/TabSeparator.gif' WIDTH='2' HEIGHT='15' ALIGN='ABSMIDDLE' BORDER='0'>"
        sNewTab = sNewTab & "</td>"
        Response.Write(sNewTab)
    end if
...
Private Function ServeContextHelp()
	...
	For each objElement in objContextHelp
		...
		strURL = strURL & "?" & SAI_FLD_PAGEKEY & "=" & Server.URLEncode(SAI_GetPageKey()) & _
