	...
	Function ServeTab1(ByRef PageIn, ByVal bIsVisible)
		...
		%>
			<TABLE WIDTH=300 VALIGN=middle ALIGN=left BORDER=0 CELLSPACING=0 CELLPADDING=2 class="TasksBody">
				<TR>
					<TD>
						<INPUT TYPE="CHECKBOX"   NAME ="chkEnableTelnetAccess"  VALUE="<%=Server.HTMLEncode(F_strTelnetSvc)%>" <%=Server.HTMLEncode(F_strEnableTelnetAccess)%> onclick="storeEnableAccessVals()">
		...
		<%else%>
			...
			<INPUT TYPE="hidden" NAME ="chkEnableTelnetAccess"  VALUE="<%=Server.HTMLEncode(F_strTelnetSvc)%>">
