...
Function OnPostFile()	
	...
	WriteLine("<INPUT type=hidden name=txtFullyQualifiedFileName value='"+Server.HTMLEncode(CStr(sUploadPath + sFileName)) + "' >" )
	WriteLine("<INPUT type=hidden name=txtFileName value='"+Server.HTMLEncode(CStr(sFileName)) + "' >" )
	WriteLine("<INPUT type=hidden name=txtFileSize value='"+Server.HTMLEncode(CStr(iFileSize)) + "' >" )
	WriteLine("</FORM>")
...
Function EmitFileSelectionForm()
	...
	WriteLine("<input type='file' name=fileSoftwareUpdate id=fileSoftwareUpdate value='"+Server.HTMLEncode(F_FileName)+"' >")
