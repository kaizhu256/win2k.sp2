	...
	Function ServetoListBox(strInput)
		...
		for nIndex = 1 to ubound(arrInput)
			...
			if instr(arrInput(nIndex),chr(2)) = 0 then
				Response.write "<OPTION VALUE=" & Chr(34) & Server.HTMLEncode(arrInput(nIndex)) & Chr(34) & "> " _
				 & Server.HTMLEncode(arrInput(nIndex)) &"</OPTION>"
			else			
				arrTemp = split(arrInput(nIndex),chr(2))								
				Response.write "<OPTION VALUE=" & Chr(34) & Server.HTMLEncode(arrTemp(0)) & Chr(34) & "> " _
					 & Server.HTMLEncode(arrTemp(1)) &"</OPTION>"
			end if
		next
	End Function
