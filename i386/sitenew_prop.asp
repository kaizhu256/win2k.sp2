<%
	...
	Dim F_strWebRootDir				'holds website root path
	...
	Dim F_strCreatePathChecked		'holds value to create the direcrory path
	Dim F_strAccountLocation		'holds value to account location
	...
	<script language="Javascript">
		...
		function GenValidatePage()
		{
			...
			document.frmTask.hdnchkCreatePath.value = 
					document.frmTask.chkCreatePath.checked;		
		    return true;
		}		
		...
		{			
			...
			for(i=0;i<2;i++)
			{
				if(document.frmTask.radAccountLocation[i].checked == true)
				{
					document.frmTask.hdnAccountLocation.value = 
						document.frmTask.radAccountLocation[i].value;
				}
			}
		...
		function ClickLocalUser()
		{
			GenerateAdmin();
			document.frmTask.txtSiteAdmin.readOnly = true;
			document.frmTask.txtSiteAdmin.style.backgroundColor = "silver";
			document.frmTask.txtAdminPwd.disabled = false;
			document.frmTask.txtAdminPwd.style.backgroundColor = "white";
			document.frmTask.txtConfirmPswd.disabled = false;
			document.frmTask.txtConfirmPswd.style.backgroundColor = "white";
			document.frmTask.txtAdminPwd.focus();
		}
		function ClickExistUser()
		{
			document.frmTask.txtSiteAdmin.readOnly = false;
			document.frmTask.txtSiteAdmin.style.backgroundColor = "white";
			document.frmTask.txtSiteAdmin.value = "";
			document.frmTask.txtAdminPwd.disabled = true;
			document.frmTask.txtAdminPwd.style.backgroundColor = "silver";
			document.frmTask.txtConfirmPswd.disabled = true;
			document.frmTask.txtConfirmPswd.style.backgroundColor = "silver";
			document.frmTask.txtSiteAdmin.focus();
		}
	...
	Function Generalviewtab()
		if GetWebSiteRootVal(F_strWebRootDir) <> CONST_SUCCESS then

			SA_TraceOut "sitenew_prop", "Unable to get the web site root directory"
	...
	%>
		...
		<TABLE WIDTH=518 VALIGN=middle ALIGN=left BORDER=0 CELLSPACING=0 CELLPADDING=2 class="TasksBody" >
			...
			<TR>
				...
				<TD class="TasksBody" colspan=2>
					<input type=text class="formField" NAME="txtSiteID"  MAXLENGTH="12" SIZE="30" value="<%=server.HTMLEncode(F_strSiteID)%>" onKeyPress = "GenerateDir(this); ClearErr();" onblur="GenerateAdmin();">
				</TD>
			</TR>
			<TR>
				<TD class="TasksBody">
				...
				<TD class="TasksBody">
					<input type=text class="formField" NAME="txtDir"  SIZE="30"  value="<%=server.HTMLEncode(F_strDir)%>" onKeyPress = "ClearErr();">
				</TD>
				<TD class="TasksBody" nowrap>
					<% if server.HTMLEncode(F_strCreatePathChecked) = "true" then %>
					<input type=checkbox class="formField" NAME="chkCreatePath" checked>
					<% else	%>
						<input type=checkbox class="formField" NAME="chkCreatePath">
					<% end if %>
			...
			<TR>
				<%If F_strAccountLocation = "1" Then %>
					<TD class="TasksBody" colspan=3>
						<input type=radio class="FormRadioButton" NAME="radAccountLocation"  value="1" checked onclick = "ClickLocalUser();">
						<%=L_CREATE_LOCAL_USER%>
					</TD>
				<%Else%>
					<TD class="TasksBody" colspan=3>
						<input type=radio class="FormRadioButton" NAME="radAccountLocation"  value="1" onclick = "ClickLocalUser();">
						<%=L_CREATE_LOCAL_USER%>
					</TD>
				<%End If%>
			</TR>
			<TR>
				<%If F_strAccountLocation = "2" Then %>
					<TD class="TasksBody" colspan=3>
						<input type=radio class="FormRadioButton" NAME="radAccountLocation"  value="2" checked onclick = "ClickExistUser();">
						<%=L_CREATE_EXIST_USER%>
					</TD>
				<%Else%>
					<TD class="TasksBody" colspan=3>
						<input type=radio class="FormRadioButton" NAME="radAccountLocation"  value="2" onclick = "ClickExistUser();">
						<%=L_CREATE_EXIST_USER%>
					</TD>
				<%End If%>
			</TR>
			<%If F_strAccountLocation <> "2" Then%>
				...
				<TR>
					...
					<TD class="TasksBody" colspan = 2>
						<input type=password class="formField" NAME="txtAdminPwd" MAXLENGTH="127" SIZE="30"   value="<%=server.HTMLEncode(F_strAdminPswd)%>" OnKeypress="ClearErr();" onfocus="this.select();">
					</TD>
				</TR>
				<TR>
					<TD class="TasksBody" nowrap>
					...
					<TD class="TasksBody" colspan = 2>
						<input type=password class="formField" NAME="txtConfirmPswd" MAXLENGTH="127" SIZE="30" value="<%=server.HTMLEncode(F_strConfirmPswd)%>" onKeyPress = "ClearErr();" onfocus="this.select();">
					</TD>
				</TR>
			<%Else%>
				<TR>
					...
					<TD class="TasksBody" colspan = 2>
						<input type=text class="formField" NAME="txtSiteAdmin"   MAXLENGTH="256" SIZE="30"   value="<%=server.HTMLEncode(F_strAdminName)%>"  OnKeypress="checkKeyforCharacters(this)">
					</TD>
				</TR>
				<TR>
					<TD class="TasksBody" nowrap>
					...
					<TD class="TasksBody" colspan = 2>
						<input type=password class="formField" NAME="txtAdminPwd" style="background-color:silver" readonly MAXLENGTH="127" SIZE="30"   value="<%=server.HTMLEncode(F_strAdminPswd)%>" OnKeypress="ClearErr();" onfocus="this.select();">
					</TD>
				</TR>
				<TR>
					<TD class="TasksBody" nowrap>
					...
					<TD class="TasksBody" colspan = 2>
						<input type=password class="formField" NAME="txtConfirmPswd" style="background-color:silver" readonly  MAXLENGTH="127" SIZE="30" value="<%=server.HTMLEncode(F_strConfirmPswd)%>" onKeyPress = "ClearErr();" onfocus="this.select();">
					</TD>
				</TR>
		...
		<input type=hidden name=hdnWebRootDir value = "<%=server.HTMLEncode(F_strWebRootDir)%>" >		
		<input type=hidden name=hdnAccountLocation value = "<%=server.HTMLEncode(F_strAccountLocation)%>" >		
		<input type=hidden name=hdnchkCreatePath value="<%=server.HTMLEncode(F_strCreatePathChecked)%>">
	...
	Function SetGenFormVariables()
		...
		F_strCreatePathChecked	= Request.Form("hdnchkCreatePath")
		F_strAccountLocation = Request.Form("hdnAccountLocation")
		
		'Call SA_TRACEOUT("SetGenFormVariables","F_strAccountLocation="&F_strAccountLocation)
	End Function
	...
	Dim F_strchkFront
	Dim F_strchkAllow
	Dim F_strCreateFTPSite			'holds value to create FTP site
	...
	Function DisplayDefaultPageItems()
		...
		For i=0 to ubound(arrItems)
			If (F_selectDefaultPage = CStr(i)) then
				Response.Write("<OPTION value="&chr(34)&CStr(i)&chr(34)&" selected>"&server.HTMLEncode(arrItems(i))&"</OPTION>")
			Else
				Response.Write("<OPTION value="&chr(34)&CStr(i)&chr(34)&">"&server.HTMLEncode(arrItems(i))&"</OPTION>")
			End If
		Next		
	End Function
