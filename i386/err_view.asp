	...
	<script language="JavaScript">
		function ClickClose() {
			...
			if (0 > sURL.indexOf('Tab1')){
				var sTab1 = '<%=FormatJScriptString(GetTab1())%>';
				var sTab2 = '<%=FormatJScriptString(GetTab2())%>';
				if ( sTab1.length > 0 ) {
					sURL += "?Tab1=" + sTab1;
					if ( sTab2.length > 0 ) {
						sURL += "&Tab2=" + sTab2;
...
<form name="frmPage" action="<% =GetScriptFileName() %>" method="POST">
	<INPUT type=hidden name="ReturnURL" value="<% =Server.HTMLEncode(mstrReturnURL) %>">
	<INPUT type=hidden name="Message" value="<% =Server.HTMLEncode(Message) %>">
