...
Function SA_RenderTree(ByVal sContainer)
	...
	Call WriteLine("<input type='hidden' name='" & SAI_FLD_PAGEKEY & "' value=""" & Server.HTMLEncode(SAI_GetPageKey()) & """ >")
	Call WriteLine("<input type='hidden' name=fieldSelectedElement value="""+Server.HTMLEncode(Request.Form("fieldSelectedElement"))+""" >")
	Call WriteLine("<input type='hidden' name=fieldImageLeaf value="""+Server.HTMLEncode(GetImageParam("fieldImageLeaf"))+""" >")
	Call WriteLine("<input type='hidden' name=fieldImageNodeOpened value="""+Server.HTMLEncode(GetImageParam("fieldImageNodeOpened"))+""" >")
	Call WriteLine("<input type='hidden' name=fieldImageNodeClosed value="""+Server.HTMLEncode(GetImageParam("fieldImageNodeClosed"))+""" >")
	Call WriteLine("</form>")
...
Function SA_RenderTreeNodes(ByVal sContainer, ByVal sSelectedHelpTOC)
	...
	If ( oContainer.Count > 0 ) Then
		...
		For each oElement in oContainer
			...
			WriteLine(" >")
			WriteLine("<img src='"+Server.HTMLEncode(sImageURL)+"' >")
			WriteLine(sCaption)
			WriteLine("</SPAN><BR>")
