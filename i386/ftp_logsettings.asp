		...
		<script language="JavaScript">	
		 function ValidatePage() 
			{ 
		 		...
		 		var FTPinst = "<%=FormatJScriptString(FTPInstalled)%>";			  
				if (FTPinst == "True")
				{					
					var strID = "";
			...
			{   	
				...
				{					
					...
					var enableLogging = "<%= FormatJScriptString(F_chkEnableLogging) %>";
			...
			<tr>
				...
				<td colspan=5 class="TasksBody">
					...
					<%if F_selectActiveFormat = CONST_ODBCLOGFILE_FORMAT then %>
						<input type=text disabled name=txtLogFileDir class="formField" tabIndex=11 onKeyPress= "ClearErr();" size=40  value="<%=Server.HTMLEncode(F_LogFileDir)%>">
					<%else%>
						<input type=text name=txtLogFileDir class="formField"  size=40 tabIndex=11 onKeyPress= "ClearErr();"value="<%=Server.HTMLEncode(F_LogFileDir)%>">
					<%end if%>
				</td>
			</tr>
		</table>
