...
%>
        <table border=0>
            ...
            <tr>
                ...
                <td nowrap class = "TasksBody">
                    <%=L_PORT_TEXT%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="txtNonEncryptedPort"  class="FormField" size="15" maxlength = "5" value="<%=Server.HTMLEncode(F_nNonEncryptedPort)%>" OnKeypress="javascript:checkKeyforNumbers(this);" >  
                </td>
            </tr>
            <tr>
                <td class = "TasksBody">
                ...
                <td class = "TasksBody">
                    <%=L_SSLPORT_TEXT%>&nbsp;&nbsp;&nbsp;<input type="text" name="txtEncryptedPort"  class="FormField" size="15" maxlength = "5" <%=Server.HTMLEncode(F_strSSLPort_Status)%> value="<%=F_nEncryptedPort%>" OnKeypress="javascript:checkKeyforNumbers(this);" >  
                </td>
            </tr>
            <tr>
                <td class = "TasksBody" colspan =2>
        ...
        <input type=hidden name=hdnNonEncryptedIPAddress value ="<%=Server.HTMLEncode(F_strNonEncryptedIPAddress)%>">
        <input type=hidden name=hdnEncryptedIPAddress value ="<%=Server.HTMLEncode(F_strEncryptedIPAddress)%>">
        <input type=hidden name=hdnNonencryptedPort value ="<%=Server.HTMLEncode(F_nNonEncryptedPort)%>">
        <input type=hidden name=hdnEncryptedPort value ="<%=Server.HTMLEncode(F_nEncryptedPort)%>">
        <input type=hidden name=hdnIPAddressstatus value ="<%=Server.HTMLEncode(F_strradIPAdd)%>">    
        <input type=hidden name=hdnSSLPortstatus value ="<%=Server.HTMLEncode(F_strSSLPort_Status)%>">    
...
%>
        ...
        <script language="JavaScript">
            ...
            function Init() 
            {    
                var strNonEncryptedIPAddress = "<%=FormatJScriptString(F_strNonEncryptedIPAddress)%>"
                var strEncryptedIPAddress = "<%=FormatJScriptString(F_strEncryptedIPAddress)%>"
...
<%
    ...
    Function SetAdminConfig(G_objAdminService,G_objService)
        ...
        If CONST_OSNAME_XPE = GetServerOSName() Then                
            strReturnURL = "/Admin/tasks.asp?" & SAI_FLD_PAGEKEY & "=" & Server.URLEncode(SAI_GetPageKey()) & "&tab1="        
        Else
            strReturnURL = "/Admin/tasks.asp?" & SAI_FLD_PAGEKEY & "=" & Server.URLEncode(SAI_GetPageKey()) & "&tab1="        
        End If    
...
%>
            <SCRIPT LANGUAGE="JAVASCRIPT" >
            {
                top.location.href = "<%=FormatJScriptString(mstrReturnURL)%>"                    
            }
            </SCRIPT>
<%    
            Response.Flush            
