...
%>
		...
		{	
			...
			if((document.frmTask.radiodomainorworkgroup[1].checked==true)
			   &&
				(Trim(strUser)=="") && ((Trim(strDomain) !="<%=FormatJScriptString(F_strDomain)%>")
				||(strDeviceName !="<%=FormatJScriptString(F_strDeviceName)%>")))
			{
					EnableUserPasswordControls();
					document.frmTask.txtdomainusername.style.backgroundColor="";
					document.frmTask.pwdomainuserpw.style.backgroundColor="";
		...
		<input type="hidden" name="hdnOriginalStatus" value="<%=Server.HTMLEncode(F_strOriginalStatus)%>">
		<input type="hidden" name="hdnRebootState" value="<%=Server.HTMLEncode(F_strRebootState)%>">
