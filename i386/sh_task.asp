...
<% 
			Call ServeStandardHeaderBar(SAI_GetBannerText(), mstrIconPath) 
%>
				<div class="PageTitleText"><% =Server.HTMLEncode(mstrPageTitle) %></div>
				</div>
		    </TD>
		</TR>
...
End If
%>
<FORM name="frmTask" onSubmit="return Next();" action="<% =Server.HTMLEncode(GetScriptFileName) %>" method="POST">
<INPUT name="<%=SAI_FLD_PAGEKEY%>" type="hidden" value="<%=Server.HTMLEncode(SAI_GetPageKey())%>">
<INPUT name="PageName" type="hidden" value="<% =Server.HTMLEncode(mstrPageName) %>">
<INPUT name="Method" type="hidden" value="<% =Server.HTMLEncode(mstrMethod) %>">
<INPUT name="ReturnURL" type="hidden" value="<% =Server.HTMLEncode(mstrReturnURL) %>">
<INPUT name="TaskType" type="hidden" value="<% =Server.HTMLEncode(mstrTaskType) %>">
<INPUT name="PageType" type="hidden" value="<% =Server.HTMLEncode(mstrWizardPageType) %>">
<INPUT name="TabSelected" type="hidden" value="<% =Server.HTMLEncode(mintTabSelected)%>">
<INPUT name="Tab1" type="hidden" value="<% =Server.HTMLEncode(GetTab1()) %>">
<INPUT name="Tab2" type="hidden" value="<% =Server.HTMLEncode(GetTab2()) %>">
<%		
...
Function ServeTaskFooter()
	...
	If GetErrMsg <> "" Then
		ErrMessage = "<table class='ErrMsg'><tr><td><img src='" & m_VirtualRoot & "images/critical_error.gif' border=0></td><td>" & Server.HTMLEncode(GetErrMsg) & "</td></tr></table>"
		SetErrMsg ""
	else
		ErrMessage =""
	End If
