...
<%
    ...
    Public Function SA_ServeFileExplorer( ByVal ExploreOptions,_
        ...
        Response.Write("<IFRAME border=0 frameborder=0 name=IFrameFSExplorer src='"+sExplorerURL+"' WIDTH='"+Server.HTMLEncode(iWidth)+"' HEIGHT='"+Server.HTMLEncode(iHeight)+"'>")
    ...
    Public Function SA_ServeResourceStatus(ByVal sImage, ByVal sCaption, ByVal sHoverText, ByVal sURL, ByVal sURLTarget, ByVal sStatusInfo)
        ...
        If ( Len(sURLTarget) <= 0 ) Then
            ...
            If ( Len(sDefaultTarget) > 0 ) Then
                sDefaultTarget = " target='" + Server.HTMLEncode(sDefaultTarget) + "' "
            End If
        Else
            sDefaultTarget =  " target='" + Server.HTMLEncode(sURLTarget) + "' "
        End If
        ...
        If ( Len(sURL) > 0 ) Then
            ...
            Response.Write(Server.HTMLEncode(sDefaultTarget))
            ...
            Response.Write(Server.HTMLEncode(sCaption))
            Response.Write("</A>")
        ...
        End If
%>
    <TD class=StatusPageStatus align=right><%=Server.HTMLEncode(sStatusInfo)%></TD>
<%  
    End Function
    ...
    Public Function SA_ServeAlertsPanel(ByVal sAlertDefContainer, ByVal sPageTitle, ByVal sWidthAttr, ByVal sHeightAttr, ByVal sDetailsURLTarget)
        ...
        Response.Write("<iframe src='"+sURL+"' border=0 frameborder=0 name=IFStatusAlerts width='"+Server.HTMLEncode(sWidthAttr)+"' height='"+Server.HTMLEncode(sHeightAttr)+"' ></iframe>")
    End Function
...
<%      
        ...
        Else 
            If ( InStr(strBackURL, "://") ) Then
                Response.Write(" onClick=""OpenNormalPage('', '"+FormatJScriptString(strBackURL)+"');"" >")
            Else
                Response.Write(" onClick=""OpenNormalPage('"+m_VirtualRoot+"', '"+FormatJScriptString(strBackURL)+"');"" >")
            End If
        End If
...
%>      
    ...
    Public Function SA_ServeOpenPageButtonEx(ByVal enPageType, _
        ...
        Select Case enPageType
            Case PT_AREA
                sOpenPage = "onClick=""SA_OnOpenNormalPage('"+m_VirtualRoot+"', '"+FormatJScriptString(sURL)+"', '"+FormatJScriptString(sReturnURL)+"'); "" "
                
            Case PT_PROPERTY
                sOpenPage = "onClick=""SA_OnOpenPropertyPage('"+m_VirtualRoot+"', '"+FormatJScriptString(sURL)+"', '"+FormatJScriptString(sReturnURL)+"', '"+FormatJScriptString(sPageTitle)+"'); "" "
...
%>
        ...
        <!-- Copyright (c) Microsoft Corporation.  All rights reserved.-->
            <head>
                ...
                function LoadPage() {
                    top.location='<%= FormatJScriptString(sURL)%>';
                }
                </SCRIPT>
            </head>
...
%>
    ...
    <!-- Copyright (c) Microsoft Corporation.  All rights reserved.-->
        <head>
            ...
            function LoadPage() {
    <%  If Trim(URL) <> "" Then %>
                top.hidden.SetupPage("<% =FormatJScriptString(URL) %>?R=" + Math.random());
    <%  Else %>
        ...
        <!-- Copyright (c) Microsoft Corporation.  All rights reserved.-->
            ...
            <BODY onLoad="Redirect();">
                <form id="frmError" method="post">
                    <INPUT name="<%=SAI_FLD_PAGEKEY%>" type="hidden" value="<%=Server.HTMLEncode(SAI_GetPageKey())%>">
                </form>
            </BODY>
        </html>
<%
